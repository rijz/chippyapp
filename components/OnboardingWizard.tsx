...
export const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ 
  ...
  const handleStartScan = async () => {
    ...
    const logInterval = setInterval(() => {
      const messages = [
        ...
        "Parsing Pricing Tables...",
        "Extracting Business Hours...",
        "Reviewing Legal & Policy Pages..."
      ];
      if (!scannedData) {
        const randomMsg = messages[Math.floor(Math.random() * messages.length)];
        addLog(randomMsg, 'processing');
      }
    }, 1200);
    ...
    try {
      ...
      if (data) {
        ...
        addLog("Scan failed. Please try manual entry.", 'pending');
        setProgress(100);
      }
    } catch (error) {
      clearInterval(progressInterval);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col lg:flex-row bg-chippy-navy font-sans">
      ...
      <div className="w-full lg:w-[450px] bg-chippy-navy text-white flex flex-col relative shrink-0 border-r border-chippy-navy-light transition-all duration-500 ease-in-out z-20">
         {/* Background Effects */}
            ...
            {step === 2 && (
               <div className="h-full flex flex-col p-12">
                  ...
                  <div className="mt-8 h-14">
                     {scannedData && (
                        <button onClick={() => setStep(3)} className="w-full h-full bg-chippy-coral hover:bg-chippy-coral-hover text-white font-bold text-lg rounded-xl transition-all shadow-lg hover:shadow-chippy-coral/20 hover:-translate-y-0.5 flex items-center justify-center gap-2 animate-in slide-in-from-bottom-2 fade-in">Proceed to Review <ArrowRight className="w-5 h-5" /></button>
                     )}
                  </div>
               </div>
            )}
...
const StatusItem = ({ label, status, icon, count }: { label: string, status: 'pending' | 'loading' | 'complete', icon: React.ReactNode, count?: number }) => (
