...
export const ChatWidget: React.FC<ChatWidgetProps> = ({
  ...
  const allowMultiLocationVoiceInDev = import.meta.env.DEV;
  const isSingleLocationVoiceEligible = locations.length <= 1 || allowMultiLocationVoiceInDev;
  ...
  const liveTranscriptFinishedRef = useRef(false);
...
${contactReqs.length > 0 ? contactReqs.map(r => `- ${r}`).join('\n') : "No details required."}
  ...
  const waitForTranscript = async (timeoutMs = 2600): Promise<string> => {
    const start = Date.now();
    while (Date.now() - start < timeoutMs) {
      const liveTranscript = sanitizeInput(liveTranscriptRef.current || '');
      const browserTranscript = sanitizeInput(browserSpeechTranscriptRef.current || '');
      const transcript = liveTranscript || browserTranscript;
      if (transcript && liveTranscriptFinishedRef.current) return transcript;
      if (transcript && Date.now() - start > 1200) return transcript;
      await new Promise(resolve => setTimeout(resolve, 120));
    }
    return sanitizeInput(liveTranscriptRef.current || browserSpeechTranscriptRef.current || '');
  };
  ...
  const startVoiceCapture = async () => {
    ...
    liveTranscriptFinishedRef.current = false;
    ...
    try {
      ...
      const session = await ai.live.connect({
        ...
        callbacks: {
          onmessage: (event: LiveServerMessage) => {
            const inputTranscriptData = event.serverContent?.inputTranscription;
            const inputTranscript = inputTranscriptData?.text?.trim();
            if (inputTranscript) {
              const prev = liveTranscriptRef.current.trim();
              let merged = inputTranscript;
              if (prev) {
                if (inputTranscript.startsWith(prev)) {
                  merged = inputTranscript;
                } else if (prev.startsWith(inputTranscript)) {
                  merged = prev;
                } else {
                  merged = `${prev} ${inputTranscript}`.trim();
                }
              }
              liveTranscriptRef.current = merged;
              setLiveTranscriptPreview(merged);
            }
            if (inputTranscriptData?.finished) {
              liveTranscriptFinishedRef.current = true;
            }
      ...
      if (SpeechRecognitionCtor) {
        ...
        recognition.lang = (tenantConfig as any)?.language || 'en-US';
        recognition.onresult = (event: any) => {
          ...
          if (finalChunk) {
            browserSpeechTranscriptRef.current = `${browserSpeechTranscriptRef.current} ${finalChunk}`.trim();
          }

          const preview = sanitizeInput(
            liveTranscriptRef.current ||
            `${browserSpeechTranscriptRef.current} ${interimChunk}`.trim()
          );
          if (preview) {
        ...
        recognition.onerror = () => {
          // Live API transcription is still the primary path.
        };
        recognition.onend = () => {
          // No-op: user can still stop via widget button.
        };
  ...
  const stopVoiceCapture = async () => {
    ...
    const transcript = await waitForTranscript(4500);
    ...
    if (!transcript) {
      ...
      if (voiceChunkCountRef.current === 0 || durationMs < 700) {
        setVoiceError('No microphone audio detected. Please check mic permission and speak for at least a second.');
      } else {
        setVoiceError('I could not hear that clearly. Please try again and speak a little slower.');
      }
      return;
    }
