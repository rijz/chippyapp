...
export const syncChatSessions = async (sessions: ChatSessionRecord[], userId: string) => {
  ...
  try {
    const records = sessions.map(s => ({
      ...
      first_response_ms: s.firstResponseMs ?? null,
      avg_response_ms: s.avgResponseMs ?? null,
      feedback_rating: s.feedbackRating ?? null,
      feedback_comment: s.feedbackComment ?? null,
      feedback_sentiment: s.feedbackSentiment ?? null,
      feedback_created_at: s.feedbackCreatedAt ? s.feedbackCreatedAt.toISOString() : null,
...
export const fetchChatSessions = async (userId: string): Promise<ChatSessionRecord[]> => {
  try {
    ...
    return (data || []).map(row => ({
      ...
      firstResponseMs: row.first_response_ms ?? undefined,
      ...
      feedbackSentiment: row.feedback_sentiment ?? undefined,
      feedbackCreatedAt: row.feedback_created_at ? new Date(row.feedback_created_at) : undefined,
